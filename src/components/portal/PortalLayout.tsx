      ...
      <aside className="hidden lg:flex w-64 shrink-0 flex-col border-r border-sidebar-border bg-sidebar text-sidebar-foreground">
        ...
        <nav className="flex-1 p-3 space-y-1">
          {nav.map((item) => {
            ...
            return (
              ...
              >
                <span className="grid h-7 w-7 place-items-center rounded-md bg-background/70 text-current shadow-sm">{item.icon}</span>
                {item.label}
              </Link>
            );
          })}
        ...
        <div className="border-t border-sidebar-border p-3">
          <div className="flex items-center gap-3 rounded-md px-2 py-2">
            <Avatar className="h-9 w-9"><AvatarFallback className="bg-primary text-primary-foreground">{user.initials}</AvatarFallback></Avatar>
            <div className="min-w-0 flex-1">
              <div className="truncate text-sm font-semibold">{user.name}</div>
              <div className="truncate text-xs text-muted-foreground">{user.sub}</div>
            </div>
            <button aria-label="Sign out" className="grid h-8 w-8 place-items-center rounded-md text-muted-foreground hover:bg-accent">
              <LogOut className="h-4 w-4" />
            </button>
          </div>
          <Link to="/" className="mt-2 block rounded-md border border-border bg-background px-3 py-2 text-center text-xs font-medium text-foreground hover:bg-accent">Back to website</Link>
        </div>
      </aside>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-30 flex items-center justify-between gap-3 border-b border-border bg-background/85 px-4 py-3 backdrop-blur md:px-6">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <Badge variant="secondary" className="hidden md:inline-flex">{role}</Badge>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button className="relative grid h-9 w-9 place-items-center rounded-md border border-border bg-background hover:bg-accent" aria-label="Notifications">
              <Bell className="h-4 w-4" />
              <span className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full bg-primary" />
            </button>
            <Avatar className="h-9 w-9 lg:hidden"><AvatarFallback className="bg-primary text-primary-foreground">{user.initials}</AvatarFallback></Avatar>
          </div>
        </header>
        ...
        <nav className="lg:hidden flex gap-1 overflow-x-auto border-b border-border bg-background px-3 py-2">
          {nav.map((item) => {
            const active = pathname === item.to;
            return (
              <Link key={item.to} to={item.to} className={`shrink-0 rounded-md px-3 py-1.5 text-xs font-medium ${active ? "bg-primary text-primary-foreground" : "bg-secondary text-foreground/70"}`}>
                {item.label}
              </Link>
            );
          })}
