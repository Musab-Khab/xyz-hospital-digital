    ...
    >
      <div className="grid gap-6">
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <Card className="border-border lg:col-span-2"><CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="font-display text-lg font-semibold">Patient volume & revenue</h2>
                <p className="text-sm text-muted-foreground">Last 6 months</p>
              </div>
              <Badge variant="secondary" className="border border-border"><TrendingUp className="mr-1 h-3 w-3 text-primary" /> Trending up</Badge>
            </div>
            <div className="mt-5 h-72">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={adminMonthly} margin={{ top: 10, right: 12, left: -10, bottom: 0 }}>
                  <defs>
                  ...
                  <Area type="monotone" dataKey="patients" stroke="#ea7a3a" strokeWidth={2} fill="url(#g1)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </CardContent></Card>
          ...
          <Card className="border-border"><CardContent className="p-6">
            ...
            <div className="mt-4 h-64">
              <ResponsiveContainer>
                <PieChart>
                  <Pie data={departmentLoad} dataKey="value" nameKey="name" innerRadius={50} outerRadius={88} paddingAngle={2}>
                    {departmentLoad.map((_, i) => <Cell key={i} fill={pieColors[i % pieColors.length]} />)}
                  </Pie>
                  <Tooltip />
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card className="border-border"><CardContent className="p-6">
            ...
            <div className="mt-4">
              <Table>
                <TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Specialty</TableHead><TableHead>Experience</TableHead><TableHead>Rating</TableHead><TableHead></TableHead></TableRow></TableHeader>
                <TableBody>
          ...
          <Card className="border-border"><CardContent className="p-6">
            ...
            <div className="mt-4 grid gap-3">
              {departments.map((d) => (
                <div key={d.slug} className="flex items-center justify-between rounded-lg border border-border p-3">
                  <div>
...
function Kpi({ label, value, delta, deltaPositive = true }: { label: string; value: string; delta?: string; deltaPositive?: boolean }) {
  return (
    <Card className="border-border"><CardContent className="p-5">
      <p className="text-xs uppercase tracking-wide text-muted-foreground">{label}</p>
      <p className="mt-2 font-display text-3xl font-bold">{value}</p>
      {delta && <p className={`mt-1 text-xs font-medium ${deltaPositive ? "text-emerald-600" : "text-destructive"}`}>{delta} vs last month</p>}
    </CardContent></Card>
  );
}
