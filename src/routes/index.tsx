...
import {
  Ambulance, ArrowRight, CalendarCheck, HeartPulse, Bone, Brain, Baby,
  Microscope, Stethoscope, Phone, Search, ShieldCheck, Sparkles, Star,
} from "lucide-react";
  ...
  return (
    <SiteLayout>
          ...
          <div className="lg:col-span-7">
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
              <Badge variant="secondary" className="rounded-full border border-border bg-background/70 px-3 py-1 text-xs">
                <Sparkles className="mr-1.5 h-3 w-3 text-primary" /> NABH · JCI · ISO 9001 accredited
              </Badge>
              ...
              <p className="mt-5 max-w-xl text-base text-muted-foreground md:text-lg">
                320+ board-certified specialists. 42 years of trust. One destination for everything from a routine check-up
                to the most complex quaternary care.
              </p>
              <div className="mt-7 flex flex-wrap gap-3">
                <Button asChild size="lg" className="gap-2">
                  <Link to="/appointment"><CalendarCheck className="h-4 w-4" /> Book appointment</Link>
                </Button>
                <Button asChild variant="outline" size="lg" className="gap-2">
                  <Link to="/doctors"><Search className="h-4 w-4" /> Find a doctor</Link>
                </Button>
              </div>
              <div className="mt-8 flex flex-wrap items-center gap-6 text-sm text-muted-foreground">
                <span className="inline-flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-primary" /> Insurance accepted</span>
                <span className="inline-flex items-center gap-2"><Star className="h-4 w-4 fill-primary text-primary" /> 4.9/5 from 12,000 patients</span>
              </div>
            </motion.div>
          </div>
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.1 }} className="lg:col-span-5">
            <div className="rounded-3xl border border-border bg-card p-2 shadow-xl shadow-primary/5">
              <div className="rounded-2xl bg-gradient-to-br from-accent via-card to-card p-6">
                <p className="text-xs font-semibold uppercase tracking-wider text-primary">Quick appointment</p>
                ...
                <div className="mt-5 grid gap-3">
                  {["Choose department", "Pick a doctor", "Select a time slot"].map((s, i) => (
                    <div key={s} className="flex items-center gap-3 rounded-xl border border-border bg-background p-3">
                      <div className="grid h-8 w-8 place-items-center rounded-full bg-primary/10 text-sm font-semibold text-primary">{i + 1}</div>
                      <p className="text-sm font-medium">{s}</p>
                      <ArrowRight className="ml-auto h-4 w-4 text-muted-foreground" />
                    </div>
                  ))}
                </div>
                <Button asChild className="mt-5 w-full"><Link to="/appointment">Start booking</Link></Button>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-20">
        <SectionHeader eyebrow="Centers of excellence" title="Featured departments" subtitle="Specialty programs led by nationally recognized physicians." />
        <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {departments.slice(0, 6).map((d, i) => {
            const Icon = iconMap[d.icon as keyof typeof iconMap] ?? Stethoscope;
            return (
              <motion.div key={d.slug} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.05 }}>
                <Card className="group h-full overflow-hidden border-border transition-all hover:-translate-y-1 hover:border-primary/40 hover:shadow-lg">
                  <CardContent className="p-6">
                    ...
                    <p className="mt-3 line-clamp-2 text-sm text-muted-foreground/90">{d.description}</p>
                    <div className="mt-5 flex items-center justify-between text-sm">
                      <span className="text-muted-foreground">{d.doctors} specialists</span>
                      <Link to="/departments" className="inline-flex items-center gap-1 font-medium text-primary group-hover:gap-2 transition-all">
                ...
                <Card className="h-full border-border bg-card">
                  <CardContent className="p-6">
                    ...
                    <div className="mt-6 flex items-center gap-3">
                      <Avatar><AvatarFallback className="bg-primary/15 text-primary">{t.name.slice(0,1)}</AvatarFallback></Avatar>
                      <div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-20">
        <div className="flex items-end justify-between gap-6">
          ...
          <Link to="/about" className="hidden md:inline-flex items-center gap-1 text-sm font-medium text-primary">All news <ArrowRight className="h-4 w-4" /></Link>
        </div>
        <div className="mt-10 grid gap-5 md:grid-cols-3">
          {news.map((n, i) => (
            <motion.article key={i} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.05 }} className="group">
              <Card className="h-full overflow-hidden border-border transition-all hover:border-primary/40 hover:shadow-md">
                ...
                <CardContent className="p-6">
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    <Badge variant="secondary" className="border border-border">{n.tag}</Badge>
                    <span>{n.date}</span>
                  </div>
                  <h3 className="mt-3 font-display text-lg font-semibold leading-snug group-hover:text-primary">{n.title}</h3>
      ...
      <section className="border-t border-border bg-gradient-to-br from-primary to-[color-mix(in_oklab,var(--primary)_70%,black)] text-primary-foreground">
        <div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-6 px-6 py-14 md:flex-row md:items-center">
          <div>
            <h3 className="font-display text-2xl font-bold md:text-3xl">Your health deserves an expert opinion.</h3>
            <p className="mt-1 text-primary-foreground/85">Book a same-day appointment or schedule a second-opinion consult.</p>
          </div>
          <div className="flex flex-wrap gap-3">
            <Button asChild size="lg" variant="secondary"><Link to="/appointment">Book appointment</Link></Button>
...
function SectionHeader({ eyebrow, title, subtitle, className = "" }: { eyebrow: string; title: string; subtitle?: string; className?: string }) {
  return (
