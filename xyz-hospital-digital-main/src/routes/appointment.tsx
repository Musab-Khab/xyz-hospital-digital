...
function AppointmentPage() {
  ...
  const filteredDoctors = useMemo(
    () => doctors.filter((d) => !dept || d.departmentSlug === dept),
    [dept],
  );
      ...
      <section className="border-b border-border bg-secondary/40">
        <div className="mx-auto max-w-5xl px-6 py-12">
          <Badge variant="secondary" className="border border-border bg-background">
            <Sparkles className="mr-1.5 h-3 w-3 text-primary" /> Online appointments
          </Badge>
                ...
                <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {departments.map((d) => (
                    <button
                      ...
                      className={`rounded-xl border p-4 text-left transition ${dept === d.slug ? "border-primary bg-primary/5 ring-2 ring-primary/30" : "border-border bg-card hover:border-primary/40"}`}
                    >
                ...
                <div className="mt-6 grid gap-3 md:grid-cols-2">
                  {filteredDoctors.map((d) => (
                    <button
                      key={d.id}
                      onClick={() => setDoc(d.id)}
                      className={`rounded-xl border p-4 text-left transition ${doc === d.id ? "border-primary bg-primary/5 ring-2 ring-primary/30" : "border-border bg-card hover:border-primary/40"}`}
                    >
              ...
              <div className="grid gap-6 md:grid-cols-[auto_1fr]">
                ...
                <div>
                  <SectionTitle
                    ...
                    title="Choose a time"
                    sub={date ? format(date, "EEEE, MMMM d") : "Pick a date first"}
                  />
                  <div className="mt-5 grid grid-cols-3 gap-2 sm:grid-cols-4 md:grid-cols-5">
                    {slots.map((s) => (
                      <button
                        ...
                        disabled={!date}
                        onClick={() => setTime(s)}
                        className={`rounded-md border px-3 py-2 text-sm font-medium transition disabled:opacity-40 ${time === s ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card hover:border-primary/40"}`}
                      >
                        {s}
                      </button>
                    ))}
              ...
              <Card className="border-border">
                <CardContent className="flex flex-col items-center px-6 py-14 text-center">
                  ...
                  <p className="mt-2 max-w-md text-muted-foreground">
                    A confirmation has been sent to {phone}. Reference #XYZ-
                  ...
                  <div className="mt-6 grid gap-2 rounded-xl border border-border bg-secondary/40 px-6 py-4 text-sm">
                    ...
                    <Row
                      label="When"
                      value={`${date ? format(date, "EEE, MMM d") : ""} · ${time}`}
                    />
                    <Row label="Where" value="Main campus, 1200 Wellness Ave" />
                  </div>
                  <div className="mt-7 flex flex-wrap gap-2">
                    <Button asChild>
                      <Link to="/patient">View in patient portal</Link>
                    </Button>
      ...
      {labels.map((l, i) => {
        ...
        return (
          <li key={l} className="flex items-center gap-2">
            <div
              className={`grid h-7 w-7 place-items-center rounded-full text-xs font-semibold ${state === "done" ? "bg-primary text-primary-foreground" : state === "current" ? "bg-primary/15 text-primary ring-2 ring-primary/40" : "bg-secondary text-muted-foreground"}`}
            >
              {n}
            </div>
            <span
              className={`text-sm ${state === "current" ? "font-semibold text-foreground" : "text-muted-foreground"}`}
            >
              {l}
            </span>
