...
export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Sign in to access your Pakistan Hospital patient, doctor, or admin portal.",
      },
...
const demo: Record<
  Role,
  { email: string; password: string; route: "/patient" | "/doctor" | "/admin"; label: string }
> = {
  patient: {
  ...
  doctor: {
    email: "dr.ahmed@pakistanhospital.pk",
    password: "doctor123",
    route: "/doctor",
    label: "Doctor",
  },
  admin: {
    email: "admin@pakistanhospital.pk",
    password: "admin123",
    route: "/admin",
    label: "Admin",
  },
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      try {
        ...
        // localStorage may be unavailable in private browsing; proceed silently
      }
      ...
      <div className="relative hidden lg:flex flex-col justify-between overflow-hidden bg-gradient-to-br from-primary via-primary to-orange-600 p-12 text-primary-foreground">
        ...
        <div className="relative z-10 max-w-md space-y-6">
          ...
          <p className="text-white/85">
            Access medical records, book appointments, view lab reports, and chat with specialists —
            all in one secure portal.
          </p>
          <ul className="space-y-3 text-sm">
            <li className="flex items-center gap-3">
              <ShieldCheck className="h-4 w-4" /> Bank-grade security &amp; HIPAA-aligned privacy
            </li>
            <li className="flex items-center gap-3">
              <Stethoscope className="h-4 w-4" /> 320+ specialists across 28 departments
            </li>
            <li className="flex items-center gap-3">
              <HeartPulse className="h-4 w-4" /> Trusted by 1.2M+ patients since 1984
            </li>
          </ul>
        </div>

        <p className="relative z-10 text-xs text-white/70">
          © 2026 Pakistan Hospital · Developed by Muhammad Musab
        </p>
      </div>
        ...
        <div className="w-full max-w-md">
                  ...
                  <CardContent className="p-4 text-xs text-muted-foreground">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-0.5">
                          Email: <span className="font-mono text-foreground">{demo[r].email}</span>
                        </p>
                        <p>
                          Password:{" "}
                          <span className="font-mono text-foreground">{demo[r].password}</span>
                        </p>
                      </div>
          ...
          <form onSubmit={onSubmit} className="mt-6 space-y-4">
            ...
            <p className="text-center text-sm text-muted-foreground">
              New patient?{" "}
              <Link to="/add-patient" className="font-medium text-primary hover:underline">
                Register here
              </Link>
            </p>
          </form>
