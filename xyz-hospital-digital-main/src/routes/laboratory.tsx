  ...
  head: () => ({
    meta: [
      { title: "Laboratory — Pakistan Hospital" },
      {
        name: "description",
        content:
          "Diagnostic tests, home sample collection, and online reports at Pakistan Hospital laboratory.",
      },
    ],
  }),
  component: LabPage,
...
function LabPage() {
  ...
  const total =
    cart.reduce((s, c) => s + (tests.find((t) => t.code === c)?.price ?? 0), 0) + (home ? 500 : 0);

  const toggle = (c: string) =>
    setCart((p) => (p.includes(c) ? p.filter((x) => x !== c) : [...p, c]));
  ...
  const book = () => {
    ...
    toast.success(`Booked ${cart.length} test(s) · Total Rs. ${total}`, {
      description: home
        ? "Home collection scheduled within 24 hrs."
        : "Visit the lab any time 7AM–9PM.",
    });
    setCart([]);
    setHome(false);
  };
      ...
      <section className="border-b border-border bg-gradient-to-br from-accent via-background to-background">
        <div className="mx-auto max-w-7xl px-6 py-14">
          <Badge variant="secondary" className="rounded-full">
            <FlaskConical className="mr-1 h-3 w-3 text-primary" /> NABL-equivalent accredited lab
          </Badge>
          <h1 className="mt-4 font-display text-4xl font-bold md:text-5xl">
            Laboratory & Diagnostics
          </h1>
          <p className="mt-3 max-w-2xl text-muted-foreground">
            800+ pathology tests, 6 advanced imaging modalities, and free home sample collection
            across Lahore.
          </p>
        </div>
      </section>
          ...
          <div className="grid gap-3">
            {filtered.map((t) => {
              ...
              return (
                <Card
                  key={t.code}
                  className={`border-border transition ${inCart ? "border-primary/50 bg-primary/5" : ""}`}
                >
                  <CardContent className="flex flex-wrap items-center justify-between gap-3 p-4">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 text-xs text-muted-foreground">
                        Code {t.code} · <Clock className="inline h-3 w-3" /> {t.tat}
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="font-display text-lg font-bold">Rs. {t.price}</p>
                    </div>
                    <Button
                      size="sm"
          ...
          <Card className="sticky top-24 border-border">
            <CardContent className="p-5">
              ...
              <ul className="mt-3 space-y-2 text-sm">
                {cart.length === 0 && (
                  <li className="text-muted-foreground">No tests added yet.</li>
                )}
                {cart.map((c) => {
              ...
              <label className="mt-4 flex items-center gap-2 rounded-md border border-border bg-secondary/40 p-3 text-sm">
                ...
                />
                <Home className="h-4 w-4 text-primary" /> Home sample collection{" "}
                <span className="ml-auto font-medium">Rs. 500</span>
              </label>
