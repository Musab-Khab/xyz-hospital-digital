...
export const Route = createFileRoute("/patient")({
  head: () => ({
    meta: [{ title: "Patient Portal — Pakistan Hospital" }, { name: "robots", content: "noindex" }],
  }),
          ...
          <CardContent className="p-0">
            <div className="flex items-center gap-2 border-b border-border bg-background/60 px-5 py-3">
              ...
              <h2 className="font-display text-sm font-semibold tracking-wide uppercase">
                Words of Healing · شفاء کے الفاظ
              </h2>
            </div>
            <div className="grid gap-4 p-5 md:grid-cols-2">
              <figure className="overflow-hidden rounded-xl border border-border bg-card">
                <img
                  src={ayat1}
                  alt="Quranic verse on healing - Surah Ash-Shu'ara 26:80"
                  loading="lazy"
                  width={1024}
                  height={1024}
                ...
                <figcaption className="p-4">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    Surah Ash-Shu'ara (26:80) · "اور جب میں بیمار ہوتا ہوں تو وہی مجھے شفا دیتا ہے"
                  </p>
                </figcaption>
              </figure>
              <figure className="overflow-hidden rounded-xl border border-border bg-card">
                <img
                  src={ayat2}
                  alt="Quranic verse on healing - Surah Al-Isra 17:82"
                  loading="lazy"
                  width={1024}
                  height={1024}
                  className="h-56 w-full object-cover"
                ...
                <figcaption className="p-4">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    Surah Al-Isra (17:82) · "اور ہم قرآن میں سے وہ نازل کرتے ہیں جو شفا ہے"
                  </p>
                </figcaption>
              </figure>
            </div>
                ...
                {[
                  ...
                  <li key={m.n} className="flex items-center gap-3">
                    <Avatar className="h-9 w-9">
                      <AvatarFallback className="bg-primary/15 text-primary">{m.i}</AvatarFallback>
                    </Avatar>
                    <div className="min-w-0 flex-1">
              ...
              <CardContent className="p-0">
                <DataTable
                  ...
                  rows={upcomingAppointments.map((a) => [
                    ...
                    <Button key={"b" + a.id} size="sm" variant="outline">
                      Manage
                    </Button>,
                  ])}
                />
              ...
              <CardContent className="p-0">
                <DataTable
                  ...
                  rows={prescriptions.map((p) => [
                    ...
                    p.issued,
                    p.refills,
                    <Button key={p.id} size="sm" variant={p.refills > 0 ? "default" : "outline"}>
                      {p.refills > 0 ? "Request refill" : "Request renewal"}
                    </Button>,
                  ])}
                />
              </CardContent>
              ...
              <CardContent className="p-0">
                <DataTable
                  ...
                  rows={labReports.map((l) => [
                    ...
                    <Badge
                      key={l.id}
                      variant={l.status === "Normal" ? "default" : "secondary"}
                      className={
                        l.status === "Normal"
                    ...
                    <Button key={"b" + l.id} size="sm" variant="outline" className="gap-1.5">
                      <Download className="h-4 w-4" />
                      PDF
                    </Button>,
                  ])}
                ...
                <DataTable
                  ...
                  rows={billing.map((b) => [
                    ...
                    <Button
                      key={"b" + b.id}
                      size="sm"
                      variant={b.status === "Pending" ? "default" : "outline"}
                    >
                      {b.status === "Pending" ? "Pay now" : "Receipt"}
                    </Button>,
                  ])}
                />
              </CardContent>
